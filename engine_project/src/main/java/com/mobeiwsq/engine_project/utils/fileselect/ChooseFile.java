...
public class ChooseFile {
    ...
    // 新增一个成员变量，用于存储允许选择的文件类型列表
    private ArrayList<String> allowedFileTypes = new ArrayList<>();

    // 新增一个方法，用于设置允许选择的文件类型列表
    public void setAllowedFileTypes(ArrayList<String> fileTypes) {
        this.allowedFileTypes = fileTypes;
    }

    // 修改 upListAdapter 方法，过滤不允许选择的文件
    private void upListAdapter() {
        ...
        if (files != null) {
            ...
            for (File file2 : files) {
                if (file2.isFile() && !allowedFileTypes.isEmpty()) {
                    // 获取文件类型
                    String fileType = pathGetType(file2.getPath());
                    // 检查文件类型是否在允许列表中
