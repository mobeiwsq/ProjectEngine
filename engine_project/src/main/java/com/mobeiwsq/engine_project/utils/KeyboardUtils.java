...
public class KeyboardUtils implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    public static boolean isSoftInputShow(ViewGroup rootView) {
        ...
        int space = viewHeight - rect.bottom - DensityUtils.INSTANCE.getNavigationBarHeight(rootView.getContext());
        return space > 0;
    }
    ...
    public static void toggleSoftInput() {
        InputMethodManager imm =
                (InputMethodManager) EngineConfig.app.getSystemService(Context.INPUT_METHOD_SERVICE);
        if (imm == null) {
            return;
        }
    ...
    public static void onTouchDown(MotionEvent event, Activity activity) {
        if (activity != null) {
            if (isShouldHideKeyboard(activity.getWindow(), event)) {
